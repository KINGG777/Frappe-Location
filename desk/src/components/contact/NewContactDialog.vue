            ...
            <template v-else-if="fieldOrRow.type === 'Link'">
              <div class="space-y-1.5">
                <Link
                  ...
                  :label="fieldOrRow.label"
                  :disabled="fieldOrRow.key === 'location' && !state.customer"
                  :filters="
                    fieldOrRow.key === 'location'
                      ? { company: state.customer }
                      : undefined
                  "
                />
              </div>
...
// Location options depend on the chosen Customer — if Customer changes,
// drop any previously-selected Location since it may no longer be valid.
watch(
  () => state.customer,
  () => {
    state.location = "";
