        ...
        <div class="space-y-4">
          ...
          <div class="space-y-1.5">
            ...
            <Link
              ...
              :filters="{ company: effectiveCustomer }"
            />
            <p v-if="!effectiveCustomer" class="text-sm text-ink-gray-4">
              {{ __("Select a customer first to see its locations") }}
            </p>
          </div>
...
// The customer whose locations should populate the Location dropdown —
// an already-linked customer takes priority, otherwise whatever is
// currently being selected in the Link-a-customer field above.
const effectiveCustomer = computed(
  () => linkedCustomers.value[0]?.name || state.customer || ""
);

// Only clear Location in response to the user actively picking a new
// customer — NOT when linkedCustomers first resolves async on load,
// which would otherwise wipe out a correctly-loaded existing Location.
watch(
  () => state.customer,
  (val, oldVal) => {
    if (oldVal !== undefined) {
