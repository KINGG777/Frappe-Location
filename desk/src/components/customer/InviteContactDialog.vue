      ...
      <div class="flex flex-col gap-4">
        ...
        <div class="space-y-1.5">
          ...
          <Link
            v-model="location"
            doctype="Location"
            :filters="{ company: customer.doc.name }"
            :placeholder="__('Select Location (optional)')"
          />
          <p class="text-sm text-ink-gray-4">
            {{
              __(
                "Applied to existing users added instantly. Invited users who haven't joined yet will need it set after they accept."
              )
            }}
          </p>
        </div>
...
import { Button, Dialog, FormControl, call, dayjsLocal, toast } from "frappe-ui";
...
function applyLocationToContacts(contactNames: string[]) {
  if (!location.value || !contactNames.length) return Promise.resolve();
  return Promise.all(
    ...
    // Contacts were still added successfully even if setting their
    // location fails (e.g. a permission edge case) — don't block on it,
    // just surface it so it's not silently lost.
    ...
    },
    {
      async onSuccess() {
        ...
        if (data.added.length) {
          ...
          await applyLocationToContacts(data.added);
