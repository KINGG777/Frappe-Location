...
const ticketAdditionalInfo = computed(() => {
  // Customer and Location are always shown here in the ticket details
  // panel, regardless of the "Hide from customer" flag on the Ticket
  // Type template — that flag only controls whether they're asked for
  // on the *new ticket* creation form (TicketCustomerTemplateFields.vue).
  // They're auto-derived (customer from the contact, location from the
  // contact's location), so there's no reason to ask for them up front,
  // but they're still useful, read-only context once the ticket exists.
  const fields = [
    {
      fieldname: "subject",
      label: "Subject",
    ...
    },
    {
      fieldname: "customer",
      label: "Customer",
      value: ticket.data.customer || "-",
    },
    {
      fieldname: "custom_location",
      label: "Location",
      value: ticket.data.custom_location || "-",
    },
  ];
  const custom_fields = ticket.data.template.fields
    .filter(
      (field: Field) =>
        ...
        ["subject", "team", "priority", "customer", "custom_location"].indexOf(
          field.fieldname
        ) === -1
    )
    .map((field: Field) => {
