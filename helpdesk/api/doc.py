...
def handle_default_view(doctype, _list, show_customer_portal_fields):
    ...
    if not columns:
        if doctype == "Contact":
            ...
            columns = _list.default_list_data(show_customer_portal_fields).get("columns")
        elif hasattr(_list, "default_list_data"):
            columns = _list.default_list_data().get("columns")
        else:
            # Generic fallback for any doctype whose controller doesn't
            # implement default_list_data (e.g. plain custom doctypes
            # like Location) — build columns straight from whichever
            # fields are flagged "In List View" in the doctype's own
            # metadata. This keeps list views working for new custom
            # doctypes without requiring app code changes per doctype.
            meta = frappe.get_meta(doctype)
            columns = [
                {
                    "label": "Name",
